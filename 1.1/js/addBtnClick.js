document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".product-card").forEach((producctCard) => {
    const produtcNme =
      producctCard.querySelector(".product__item")?.textContent || "";
  ...
  cardBtns.forEach((cardBtn) => {
    cardBtn.addEventListener("click", (e) => {
      ...
      const produtcNme =
        proddutc.querySelector(".product__item")?.textContent || "";
      const price__one =
        proddutc.querySelector(".price__one")?.textContent || "";
      ...
      const prductIndex = cart.findIndex(
        (item) => item.product__item === newProductCart.produtcNme
      );
      if (prductIndex === -1) {
